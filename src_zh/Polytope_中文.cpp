...
bool Polytope::contains(const osg::Vec3f& v0, const osg::Vec3f& v1, const osg::Vec3f& v2) const
{
        ...
        ++pitr)
    {
        ...
        {
            //OSG_NOTICE<<"Polytope::contains() 平面测试"<<std::endl;
            ...
            if (dest.size()<=1)
            {
                // OSG_NOTICE<<"Polytope::contains() 三角形上的所有点都被剔除, dest.size()="<<dest.size()<<std::endl;
                return false;
            }
        ...
        {
            // OSG_NOTICE<<"Polytope::contains() 平面已禁用"<<std::endl;
        }

        selector_mask <<= 1;
    }

    //OSG_NOTICE<<"Polytope::contains() 三角形在多面体内, src.size()="<<src.size()<<std::endl;
    return true;
} 